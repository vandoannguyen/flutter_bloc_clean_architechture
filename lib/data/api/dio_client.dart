...
import 'package:base_flutter_bloc/common/constant.dart';
import 'package:base_flutter_bloc/common/logger/logger.dart';
import 'package:base_flutter_bloc/data/api/url_config.dart';
import 'package:base_flutter_bloc/data/models/error/business_error.dart';
import 'package:base_flutter_bloc/data/models/token/token_info.dart';
import 'package:base_flutter_bloc/domain/exception/index.dart';
...
class DioClient {
  ...
  Future<void> _doExpire() async {
    ...
    SharedPreferenceUtils.instance().remove(AUTH_TOKEN);
  }
  ...
  Future<void> _issueNewToken(TokenInfo currentTokenInfo) async {
    try {
      ...
      await SharedPreferenceUtils.instance()
          .putString(AUTH_TOKEN, jsonEncode(newTokenInfo?.toJson()));
    } finally {
  ...
  void _onErrorInterceptor(
    ...
    if (error.type == DioExceptionType.badResponse) {
      if (error.requestOptions.path == UrlConfig.refreshToken &&
        ...
        final tokenInfo =
            SharedPreferenceUtils.instance().getString(AUTH_TOKEN);
        ...
        if (tokenInfo == null) {
          ...
          try {
            ...
            if (!_isRefreshingToken) {
              {
                await _issueNewToken(
                  TokenInfo.fromJson(
                    jsonDecode(tokenInfo),
                  ),
                );
  ...
  void _onResponseInterceptor(
    ...
    if (response.requestOptions.path == UrlConfig.login &&
      ...
      SharedPreferenceUtils.instance().putString(
        AUTH_TOKEN,
        jsonEncode(token),
      );
    }
  ...
  Future<Map<String, dynamic>> _getDefaultHeader() async {
    try {
      final token = SharedPreferenceUtils.instance().getString(AUTH_TOKEN);
      if (token == null) {
        return {
